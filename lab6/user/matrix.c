...
void work(unsigned int times)
{
    ...
    for (i = 0; i < size; i++)
    {
        for (j = 0; j < size; j++) { mata[i][j] = matb[i][j] = 1; }
    }
    ...
    while (times-- > 0)
    {
        for (i = 0; i < size; i++)
        {
            ...
            {
                matc[i][j] = 0;
                for (k = 0; k < size; k++) { matc[i][j] += mata[i][k] * matb[k][j]; }
            }
        }
        for (i = 0; i < size; i++)
        {
            for (j = 0; j < size; j++) { mata[i][j] = matb[i][j] = matc[i][j]; }
        }
    }
    cprintf("pid %d done!.\n", getpid());
    exit(0);
