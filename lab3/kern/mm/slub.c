...
static void* slob_alloc(size_t size)
{
    ...
    for (cur = prev->next;; prev = cur, cur = cur->next)
    {
        if (cur->units >= units)
        {
            if (cur->units == units) { prev->next = cur->next; }
            else
            {
                prev->next        = cur + units;
...
void slub_init(void) { cprintf("slub_init() succeeded!\n"); }
...
unsigned int slub_size(const void* block)
{
    ...
    if (!((unsigned long)block & (PGSIZE - 1)))
    {
        ...
        {
            if (bb->pages == block) { return bb->order << PGSHIFT; }
        }
    }
...
{
    int len = 0;
    for (slob_t* curr = slobfree->next; curr != slobfree; curr = curr->next) { len++; }
    return len;
}
