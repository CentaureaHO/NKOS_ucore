...
#define __test_and_op_bit(op, mod, nr, addr)                                                                      \
    ({                                                                                                            \
        ...
        __mask = BIT_MASK(nr);                                                                                    \
        __asm__ __volatile__(__AMO(op) " %0, %2, %1" : "=r"(__res), "+A"(addr[BIT_WORD(nr)]) : "r"(mod(__mask))); \
        ((__res & __mask) != 0);                                                                                  \
    })

#define __op_bit(op, mod, nr, addr) \
    __asm__ __volatile__(__AMO(op) " zero, %1, %0" : "+A"(addr[BIT_WORD(nr)]) : "r"(mod(BIT_MASK(nr))))
 ...
 * */
static inline void set_bit(int nr, volatile void* addr) { __op_bit(or, __NOP, nr, ((volatile unsigned long*)addr)); }
 ...
 * @addr:   the address to start counting from
 * */
static inline void clear_bit(int nr, volatile void* addr) { __op_bit(and, __NOT, nr, ((volatile unsigned long*)addr)); }
 ...
 * */
static inline bool test_bit(int nr, volatile void* addr) { return (((*(volatile unsigned long*)addr) >> nr) & 1); }
