...
static struct kmem_cache_t
    cache_cache;  // kmem_cache_t仓库，kmem_cache_t也是由Slab算法分配的，因此它会cache所有的kmem_cache_t
static struct kmem_cache_t* sized_caches
    [SIZED_CACHE_NUM];  // 指针数组，数组的每一个元素指向1个固定大小的内置仓库，共有8个内置仓库，这些仓库存放实际的对象
...
static void* kmem_cache_grow(struct kmem_cache_t* cachep)
{
    ...
    int16_t* bufctl = kva;
    for (int i = 1; i < cachep->num; i++) bufctl[i - 1] = i;
...
static int kmem_sized_index(size_t size)
{
    ...
    if (rsize < SIZED_CACHE_MIN) rsize = SIZED_CACHE_MIN;
    // 寻找索引
    int index = 0;
...
{
    char* p = objp;
    for (int i = 0; i < size; i++) p[i] = TEST_OBJECT_CTVAL;
}

static void test_dtor(void* objp, struct kmem_cache_t* cachep, size_t size)
{
    char* p = objp;
    for (int i = 0; i < size; i++) p[i] = TEST_OBJECT_DTVAL;
}
...
static size_t list_length(list_entry_t* listelm)
{
    ...
    while ((le = list_next(le)) != listelm) len++;
    return len;
}
...
static void check_kmem()
{
    ...
    for (int i = 0; i < sizeof(struct test_object); i++) assert(p[i] == TEST_OBJECT_CTVAL);
    assert((p5 = kmem_cache_zalloc(cp0)) != NULL);
    p = (char*)p5;
    // 由于调用的是zalloc，也就是说分配一个对象之后对象内存区域全初始化为零，所以有p[i] == 0
    for (int i = 0; i < sizeof(struct test_object); i++) assert(p[i] == 0);
    ...
    p = (char*)p4;
    for (int i = 0; i < sizeof(struct test_object); i++) assert(p[i] == TEST_OBJECT_DTVAL);
...
*/
struct kmem_cache_t* kmem_cache_create(const char* name, size_t size, void (*ctor)(void*, struct kmem_cache_t*, size_t),
...
void* kmem_cache_alloc(struct kmem_cache_t* cachep)
{
    ...
    // 1. 先查找slabs_partial
    if (!list_empty(&(cachep->slabs_partial))) le = list_next(&(cachep->slabs_partial));
    // 再查找slabs_free
    else
    {
...
// 获得仓库中对象的大小
size_t kmem_cache_size(struct kmem_cache_t* cachep) { return cachep->objsize; }

// 获得仓库的名称
const char* kmem_cache_name(struct kmem_cache_t* cachep) { return cachep->name; }
