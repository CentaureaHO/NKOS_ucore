...
static uint32_t hash32(uint32_t val) { return __hash32(val, HASH_SHIFT); }

static uint32_t hash64(uint64_t val) { return __hash32((uint32_t)val, HASH_SHIFT); }

void* safe_malloc(size_t size)
{
    void* ret;
    if ((ret = malloc(size)) == NULL) { bug("malloc %lu bytes failed.\n", (long unsigned)size); }
    return ret;
}
...
{
    char* ret;
    if ((ret = strdup(str)) == NULL) { bug("strdup failed: %s\n", str); }
    return ret;
}
...
struct stat* safe_stat(const char* filename)
{
    ...
    if (stat(filename, &__stat) != 0) { bug("stat %s failed.\n", filename); }
    return &__stat;
}
...
{
    static struct stat __stat;
    if (fstat(fd, &__stat) != 0) { bug("fstat %d failed.\n", fd); }
    return &__stat;
}
...
{
    static struct stat __stat;
    if (lstat(name, &__stat) != 0) { bug("lstat '%s' failed.\n", name); }
    return &__stat;
}

void safe_fchdir(int fd)
{
    if (fchdir(fd) != 0) { bug("fchdir failed %d.\n", fd); }
}
...
struct cache_block* search_cache_block(struct sfs_fs* sfs, uint32_t ino)
{
    ...
    while (cb != NULL && cb->ino != ino) { cb = cb->hash_next; }
    return cb;
}
...
struct cache_inode* search_cache_inode(struct sfs_fs* sfs, ino_t real)
{
    ...
    while (ci != NULL && ci->real != real) { ci = ci->hash_next; }
    return ci;
}
...
static void subpath_show(FILE* fout, struct sfs_fs* sfs, const char* name)
{
    ...
    while ((subpath = subpath->next) != NULL) { fprintf(fout, "%s/", subpath->subname); }
    if (name != NULL) { fprintf(fout, "%s", name); }
    fprintf(fout, "\n");
}
...
void close_sfs(struct sfs_fs* sfs)
{
    ...
    for (i = 0; i < ninos; ino++, i += SFS_BLKBITS)
    {
        ...
        if (i + SFS_BLKBITS > next_ino)
        {
            ...
            if (i < next_ino) { start = next_ino - i; }
            if (i + SFS_BLKBITS > ninos) { end = ninos - i; }
            uint32_t*      data = (uint32_t*)buffer;
            const uint32_t bits = sizeof(bits) * CHAR_BIT;
            for (j = start; j < end; j++) { data[j / bits] |= (1 << (j % bits)); }
        }
...
struct sfs_fs* open_img(const char* imgname)
{
    ...
    if (ext <= imgname || strcmp(ext, expect) != 0) { bug("invalid .img file name '%s'.\n", imgname); }
    int imgfd;
    if ((imgfd = open(imgname, O_WRONLY)) < 0) { bug("open '%s' failed.\n", imgname); }
...
static void append_block(struct sfs_fs* sfs, struct cache_inode* file, size_t size, uint32_t ino, const char* filename)
{
    ...
    if (nblks >= SFS_LN_NBLKS) { open_bug(sfs, filename, "file is too big.\n"); }
    if (nblks < SFS_L0_NBLKS) { inode->direct[nblks] = ino; }
...
{
    DIR* dir;
    if ((dir = opendir(".")) == NULL) { open_bug(sfs, NULL, "opendir failed.\n"); }
    ...
    while ((direntp = readdir(dir)) != NULL)
    {
        ...
        if (name[0] == '.') { continue; }
        if (strlen(name) > SFS_MAX_FNAME_LEN) { open_bug(sfs, NULL, "file name is too long: %s\n", name); }
        struct stat* stat = safe_lstat(name);
        if (S_ISLNK(stat->st_mode)) { add_link(sfs, current, name, stat->st_ino); }
        else
        {
            int fd;
            if ((fd = open(name, O_RDONLY)) < 0) { open_bug(sfs, NULL, "open failed: %s\n", name); }
            if (S_ISDIR(stat->st_mode)) { add_dir(sfs, current, name, dirfd(dir), fd, stat->st_ino); }
            else if (S_ISREG(stat->st_mode)) { add_file(sfs, current, name, fd, stat->st_ino); }
            else
            {
                char mode = '?';
...
void open_file(struct sfs_fs* sfs, struct cache_inode* file, const char* filename, int fd)
{
    ...
    if (ret < 0) { open_bug(sfs, filename, "read file failed.\n"); }
}
...
void open_link(struct sfs_fs* sfs, struct cache_inode* file, const char* filename)
{
    ...
    if (ret < 0 || ret == SFS_BLKSIZE) { open_bug(sfs, filename, "read link failed, %d", (int)ret); }
    write_block(sfs, buffer, ret, ino);
    append_block(sfs, file, ret, ino, filename);
}
...
{
    int curfd, homefd;
    if ((curfd = open(".", O_RDONLY)) < 0) { bug("get current fd failed.\n"); }
...
{
    static_check();
    if (argc != 3) { bug("usage: <input *.img> <input dirname>\n"); }
    const char *imgname = argv[1], *home = argv[2];
    if (create_img(open_img(imgname), home) != 0) { bug("create img failed.\n"); }
