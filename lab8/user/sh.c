...
{
    char* s;
    if ((s = *p1) == NULL) { return 0; }
    while (strchr(WHITESPACE, *s) != NULL) { *s++ = '\0'; }
    if (*s == '\0') { return 0; }

    *p2       = s;
    int token = 'w';
    if (strchr(SYMBOLS, *s) != NULL) { token = *s, *s++ = '\0'; }
    else
    {
        bool flag = 0;
...
char* readline(const char* prompt)
{
    ...
    {
        char c;
        if ((ret = read(0, &c, sizeof(char))) < 0) { return NULL; }
        else if (ret == 0)
        {
            if (i > 0)
            {
...
void usage(void) { printf("usage: sh [command-file]\n"); }
...
{
    int ret;
    if ((ret = open(name, O_RDONLY)) < 0) { return ret; }
    close(ret);
    return 0;
}
    ...
    while (1)
    {
        switch (token = gettoken(&cmd, &t))
        {
            ...
            case '<':
                ...
                if ((ret = reopen(0, t, O_RDONLY)) != 0) { return ret; }
                break;
            case '>':
                if (gettoken(&cmd, &t) != 'w')
                {
                ...
                if ((ret = reopen(1, t, O_RDWR | O_TRUNC | O_CREAT)) != 0) { return ret; }
                break;
            case '|':
                //  if ((ret = pipe(p)) != 0) {
                //      return ret;
                ...
                {
                    close(0);
                    if ((ret = dup2(p[0], 0)) < 0) { return ret; }
                    close(p[0]), close(p[1]);
                    goto again;
                }
                else
                {
                    if (ret < 0) { return ret; }
                    close(1);
                    if ((ret = dup2(p[1], 1)) < 0) { return ret; }
                    close(p[0]), close(p[1]);
                    goto runit;
                }
                break;
            ...
            case ';':
                ...
                break;
            default: printf("sh error: bad return %d from gettoken\n", token); return -1;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    if (argc == 2)
    {
        if ((ret = reopen(0, argv[1], O_RDONLY)) != 0) { return ret; }
        interactive = 0;
    }
    else if (argc > 2)
    {
    ...
    while ((buffer = readline((interactive) ? "$ " : NULL)) != NULL)
    {
        ...
        {
            if (ret == 0 && shcwd[0] != '\0') { ret = 0; }
            if (ret != 0) { printf("error: %d - %e\n", ret, ret); }
        }
    }
    return 0;
}
