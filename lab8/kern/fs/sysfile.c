...
static int copy_path(char** to, const char* from)
{
    ...
    char*             buffer;
    if ((buffer = kmalloc(FS_MAX_FPATH_LEN + 1)) == NULL) { return -E_NO_MEM; }
    lock_mm(mm);
    ...
    while (len != 0)
    {
        ...
        if (alen != 0)
        {
            lock_mm(mm);
            {
                ...
                else if (ret == 0) { ret = -E_INVAL; }
            }
            unlock_mm(mm);
        }
        if (ret != 0 || alen == 0) { goto out; }
    }
...
int sysfile_write(int fd, void* base, size_t len)
{
    ...
    while (len != 0)
    {
        ...
        lock_mm(mm);
        {
            if (!copy_from_user(mm, buffer, base, alen, 0)) { ret = -E_INVAL; }
        }
        unlock_mm(mm);
        if (ret == 0)
        {
...
int sysfile_seek(int fd, off_t pos, int whence) { return file_seek(fd, pos, whence); }
...
int sysfile_fstat(int fd, struct stat* __stat)
{
    ...
    if ((ret = file_fstat(fd, stat)) != 0) { return ret; }

    lock_mm(mm);
    {
        if (!copy_to_user(mm, __stat, stat, sizeof(struct stat))) { ret = -E_INVAL; }
    }
    unlock_mm(mm);
    return ret;
}
...
int sysfile_link(const char* __path1, const char* __path2)
{
    ...
    if ((ret = copy_path(&old_path, __path1)) != 0) { return ret; }
...
int sysfile_getdirentry(int fd, struct dirent* __direntp)
{
    ...
    if ((direntp = kmalloc(sizeof(struct dirent))) == NULL) { return -E_NO_MEM; }
    ...
    if (ret != 0 || (ret = file_getdirentry(fd, direntp)) != 0) { goto out; }

    lock_mm(mm);
    {
        if (!copy_to_user(mm, __direntp, direntp, sizeof(struct dirent))) { ret = -E_INVAL; }
    }
    unlock_mm(mm);
...
int sysfile_dup(int fd1, int fd2) { return file_dup(fd1, fd2); }

int sysfile_pipe(int* fd_store) { return -E_UNIMP; }

int sysfile_mkfifo(const char* __name, uint32_t open_flags) { return -E_UNIMP; }
