...
static int stdout_close(struct device* dev) { return 0; }
...
static int stdout_io(struct device* dev, struct iobuf* iob, bool write)
{
    if (write)
    {
        ...
        for (; iob->io_resid != 0; iob->io_resid--) { cputchar(*data++); }
        return 0;
    }
    return -E_INVAL;
}

static int stdout_ioctl(struct device* dev, int op, void* data) { return -E_INVAL; }
