...
struct bitmap* bitmap_create(uint32_t nbits)
{
    ...
    if ((bitmap = kmalloc(sizeof(struct bitmap))) == NULL) { return NULL; }
    ...
    if (nbits != nwords * WORD_BITS)
    {
        ...
        for (; overbits < WORD_BITS; overbits++) { bitmap->map[ix] ^= (1 << overbits); }
    }
    return bitmap;
}
...
{
    if (len_store != NULL) { *len_store = sizeof(WORD_TYPE) * bitmap->nwords; }
    return bitmap->map;
}
