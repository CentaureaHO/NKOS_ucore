...
static int sfs_rwblock(struct sfs_fs* sfs, void* buf, uint32_t blkno, uint32_t nblks, bool write)
{
    ...
    lock_sfs_io(sfs);
    {
        while (nblks != 0)
        {
            if ((ret = sfs_rwblock_nolock(sfs, buf, blkno, write, 1)) != 0) { break; }
            blkno++, nblks--;
            buf += SFS_BLKSIZE;
        }
    }
...
/* sfs_rbuf - The Basic block-level I/O routine for  Rd( non-block & non-aligned io) one disk block(using
 * sfs->sfs_buffer) with lock protect for mutex process on Rd/Wr disk block
 * @sfs:    sfs_fs which will be process
...
/* sfs_wbuf - The Basic block-level I/O routine for  Wr( non-block & non-aligned io) one disk block(using
 * sfs->sfs_buffer) with lock protect for mutex process on Rd/Wr disk block
 * @sfs:    sfs_fs which will be process
...
int sfs_clear_block(struct sfs_fs* sfs, uint32_t blkno, uint32_t nblks)
{
    ...
    lock_sfs_io(sfs);
    {
        ...
        while (nblks != 0)
        {
            if ((ret = sfs_rwblock_nolock(sfs, sfs->sfs_buffer, blkno, 1, 1)) != 0) { break; }
            blkno++, nblks--;
        }
    }
    unlock_sfs_io(sfs);
