 ...
 */
void lock_sfs_fs(struct sfs_fs* sfs) { down(&(sfs->fs_sem)); }
 ...
 */
void lock_sfs_io(struct sfs_fs* sfs) { down(&(sfs->io_sem)); }
 ...
 * called by: sfs_load_inode, sfs_sync, sfs_reclaim
 */
void unlock_sfs_fs(struct sfs_fs* sfs) { up(&(sfs->fs_sem)); }
 ...
 * called by: sfs_rwblock sfs_clear_block sfs_sync_super
 */
void unlock_sfs_io(struct sfs_fs* sfs) { up(&(sfs->io_sem)); }
