...
void fs_cleanup(void) { vfs_cleanup(); }

void lock_files(struct files_struct* filesp) { down(&(filesp->files_sem)); }

void unlock_files(struct files_struct* filesp) { up(&(filesp->files_sem)); }
...
void files_destroy(struct files_struct* filesp)
{
    ...
    if (filesp->pwd != NULL) { vop_ref_dec(filesp->pwd); }
    int          i;
    struct file* file = filesp->fd_array;
    for (i = 0; i < FILES_STRUCT_NENTRY; i++, file++)
    {
        if (file->status == FD_OPENED) { fd_array_close(file); }
...
int dup_files(struct files_struct* to, struct files_struct* from)
{
    ...
    if ((to->pwd = from->pwd) != NULL) { vop_ref_inc(to->pwd); }
    int          i;
