...
static int fd_array_alloc(int fd, struct file** file_store)
{
    ...
    if (fd == NO_FD)
    {
        for (fd = 0; fd < FILES_STRUCT_NENTRY; fd++, file++)
        {
            if (file->status == FD_NONE) { goto found; }
        }
        return -E_MAX_OPEN;
    }
    else
...
static void fd_array_free(struct file* file)
{
    ...
    assert(fopen_count(file) == 0);
    if (file->status == FD_CLOSED) { vfs_close(file->node); }
    file->status = FD_NONE;
}
...
static void fd_array_release(struct file* file)
{
    ...
    assert(fopen_count(file) > 0);
    if (fopen_count_dec(file) == 0) { fd_array_free(file); }
}
...
int file_open(char* path, uint32_t open_flags)
{
    ...
    int          ret;
    struct file* file;
    if ((ret = fd_array_alloc(NO_FD, &file)) != 0) { return ret; }
    struct inode* node;
...
int file_seek(int fd, off_t pos, int whence)
{
    ...
    switch (whence)
    {
        ...
        case LSEEK_END:
            if ((ret = vop_fstat(file->node, stat)) == 0) { pos += stat->st_size; }
            break;
        default: ret = -E_INVAL;
    }
    ...
    if (ret == 0)
    {
        if ((ret = vop_tryseek(file->node, pos)) == 0) { file->pos = pos; }
...
int file_getdirentry(int fd, struct dirent* direntp)
{
    ...
    if ((ret = vop_getdirentry(file->node, iob)) == 0) { direntp->offset += iobuf_used(iob); }
    fd_array_release(file);
    return ret;
}
...
int file_dup(int fd1, int fd2)
{
    ...
    if ((ret = fd2file(fd1, &file1)) != 0) { return ret; }
    if ((ret = fd_array_alloc(fd2, &file2)) != 0) { return ret; }
