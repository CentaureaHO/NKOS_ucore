...
#define le2vdev(le, member) to_struct((le), vfs_dev_t, member)
    ...
    if (!list_empty(&vdev_list))
    {
        lock_vdev_list();
        {
            ...
            while ((le = list_next(le)) != list)
            {
                ...
                if (vdev->fs != NULL) { fsop_cleanup(vdev->fs); }
            }
        }
        unlock_vdev_list();
    }
        ...
        lock_vdev_list();
        {
            ...
            while ((le = list_next(le)) != list)
            {
                ...
                if (strcmp(devname, vdev->devname) == 0)
                {
                    ...
                    if (vdev->fs != NULL) { found = fsop_get_root(vdev->fs); }
                    else if (!vdev->mountable)
                    {
                        vop_ref_inc(vdev->devnode);
                        found = vdev->devnode;
                    }
                    if (found != NULL) { ret = 0, *node_store = found; }
                    else { ret = -E_NA_DEV; }
                    break;
                }
            }
...
static bool check_devname_conflict(const char* devname)
{
    ...
    {
        vfs_dev_t* vdev = le2vdev(le, vdev_link);
        if (strcmp(vdev->devname, devname) == 0) { return 0; }
    }
    return 1;
}
...
static int vfs_do_add(const char* devname, struct inode* devnode, struct fs* fs, bool mountable)
{
    ...
    int   ret = -E_NO_MEM;
    char* s_devname;
    if ((s_devname = strdup(devname)) == NULL) { return ret; }

    vfs_dev_t* vdev;
    if ((vdev = kmalloc(sizeof(vfs_dev_t))) == NULL) { goto failed_cleanup_name; }
 ...
 *              mountable.
 */
int vfs_add_fs(const char* devname, struct fs* fs) { return vfs_do_add(devname, NULL, fs, 0); }
