...
{
    if (proc != idleproc) { sched_class->enqueue(rq, proc); }
}
...
void wakeup_proc(struct proc_struct* proc)
{
    ...
    local_intr_save(intr_flag);
    {
        if (proc->state != PROC_RUNNABLE)
        {
            ...
            proc->wait_state = 0;
            if (proc != current) { sched_class_enqueue(proc); }
        }
        else { warn("wakeup runnable process.\n"); }
    }
...
void schedule(void)
{
    ...
    local_intr_save(intr_flag);
    {
        ...
        if ((next = sched_class_pick_next()) != NULL) { sched_class_dequeue(next); }
    ...
    local_intr_save(intr_flag);
    {
        ...
        if (le != &timer_list)
        {
            ...
            while (timer->expires == 0)
            {
                ...
                if (proc->wait_state != 0) { assert(proc->wait_state & WT_INTERRUPTED); }
                else { warn("process %d's wait_state == 0.\n", proc->pid); }
                wakeup_proc(proc);
                del_timer(timer);
