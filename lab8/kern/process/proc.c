...
static int copy_files(uint32_t clone_flags, struct proc_struct* proc)
{
    ...
    int ret = -E_NO_MEM;
    if ((filesp = files_create()) == NULL) { goto bad_files_struct; }

    if ((ret = dup_files(filesp, old_filesp)) != 0) { goto bad_dup_cleanup_fs; }
...
static void put_files(struct proc_struct* proc)
{
    ...
    if (filesp != NULL)
    {
        if (files_count_dec(filesp) == 0) { files_destroy(filesp); }
    }
}
...
{
    int ret;
    if ((ret = sysfile_seek(fd, offset, LSEEK_SET)) != 0) { return ret; }
    if ((ret = sysfile_read(fd, buf, len)) != len) { return (ret < 0) ? ret : -1; }
    return 0;
}
...
static int load_icode(int fd, int argc, char** kargv)
{
     ...
     * argc/argv? MACROs or Functions: mm_create        - create a mm setup_pgdir      - setup pgdir in mm
...
{
    while (argc > 0) { kfree(kargv[--argc]); }
}

static int copy_kargv(struct mm_struct* mm, int argc, char** kargv, const char** argv)
{
    int i, ret = -E_INVAL;
    if (!user_mem_check(mm, (uintptr_t)argv, sizeof(const char*) * argc, 0)) { return ret; }
    for (i = 0; i < argc; i++)
    {
        char* buffer;
        if ((buffer = kmalloc(EXEC_MAX_ARG_LEN + 1)) == NULL) { goto failed_nomem; }
...
int do_execve(const char* name, int argc, const char** argv)
{
    ...
    struct mm_struct* mm = current->mm;
    if (!(argc >= 1 && argc <= EXEC_MAX_ARG_NUM)) { return -E_INVAL; }
    ...
    lock_mm(mm);
    if (name == NULL) { snprintf(local_name, sizeof(local_name), "<null> %d", current->pid); }
    else
    {
    ...
    /* sysfile_open will check the first argument path, thus we have to use a user-space pointer, and argv[0] may be
     * incorrect */
    int fd;
    if ((ret = fd = sysfile_open(path, O_RDONLY)) < 0) { goto execve_exit; }
    if (mm != NULL)
    {
        lcr3(boot_cr3);
        if (mm_count_dec(mm) == 0)
    ...
    ret = -E_NO_MEM;
    ;
    if ((ret = load_icode(fd, argc, kargv)) != 0) { goto execve_exit; }
...
int do_wait(int pid, int* code_store)
{
    ...
    if (code_store != NULL)
    {
        if (!user_mem_check(mm, (uintptr_t)code_store, sizeof(int), 1)) { return -E_INVAL; }
    }
...
{
    int ret;
    if ((ret = vfs_set_bootfs("disk0:")) != 0) { panic("set boot fs failed: %e.\n", ret); }
    ...
    while (do_wait(0, NULL) == 0) { schedule(); }
...
void proc_init(void)
{
    ...
    if ((idleproc->filesp = files_create()) == NULL) { panic("create filesp (idleproc) failed.\n"); }
