...
static void init_memmap(struct Page* base, size_t n) { pmm_manager->init_memmap(base, n); }
...
static void page_init(void)
{
    ...
    for (size_t i = 0; i < npage - nbase; i++) { SetPageReserved(pages + i); }
...
struct Page* get_page(pde_t* pgdir, uintptr_t la, pte_t** ptep_store)
{
    ...
    if (ptep_store != NULL) { *ptep_store = ptep; }
    if (ptep != NULL && *ptep & PTE_V) { return pte2page(*ptep); }
    return NULL;
}
...
void unmap_range(pde_t* pgdir, uintptr_t start, uintptr_t end)
{
    ...
    do {
        ...
        if (*ptep != 0) { page_remove_pte(pgdir, start, ptep); }
        start += PGSIZE;
 ...
 */
/* copy_range - 将进程 A 的内存内容（start 到 end）复制到另一个进程 B
 * @to:    进程 B 的页目录地址
 * @from:  进程 A 的页目录地址
 * @share: 标志位，用于指示是复制（dup）还是共享（share）。我们仅使用复制方法，因此该参数未被使用。
 *
 * 调用关系图: copy_mm --> dup_mmap --> copy_range
 */
...
{
    pte_t* ptep = get_pte(pgdir, la, 0);
    if (ptep != NULL) { page_remove_pte(pgdir, la, ptep); }
}
...
void tlb_invalidate(pde_t* pgdir, uintptr_t la) { asm volatile("sfence.vma %0" : : "r"(la)); }
...
// perm permission
static int get_pgtable_items(
    size_t left, size_t right, size_t start, uintptr_t* table, size_t* left_store, size_t* right_store)
{
    if (start >= right) { return 0; }
    while (start < right && !(table[start] & PTE_V)) { start++; }
    if (start < right)
    {
        if (left_store != NULL) { *left_store = start; }
        int perm = (table[start++] & PTE_USER);
        while (start < right && (table[start] & PTE_USER) == perm) { start++; }
        if (right_store != NULL) { *right_store = start; }
        return perm;
    }
    return 0;
}
