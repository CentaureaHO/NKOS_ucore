...
struct pmm_manager
{
    ...
    void (*free_pages)(
        struct Page* base, size_t n);  // free >=n pages with "base" addr of Page descriptor structures(memlayout.h)
...
#define PADDR(kva)                                                                         \
    ({                                                                                     \
        ...
        if (__m_kva < KERNBASE) { panic("PADDR called with invalid kva %08lx", __m_kva); } \
...
#define KADDR(pa)                                                                      \
    ({                                                                                 \
        ...
        if (__m_ppn >= npage) { panic("KADDR called with invalid pa %08lx", __m_pa); } \
...
static inline struct Page* pde2page(pde_t pde) { return pa2page(PDE_ADDR(pde)); }
