...
int swap_set_unswappable(struct mm_struct* mm, uintptr_t addr) { return sm->set_unswappable(mm, addr); }
...
static void check_swap(void)
{
    ...
    nr_free                    = 0;
    for (i = 0; i < CHECK_VALID_PHY_PAGE_NUM; i++) { free_pages(check_rp[i], 1); }
